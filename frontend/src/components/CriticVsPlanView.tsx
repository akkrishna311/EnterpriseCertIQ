  ...
  return (
    <div className={clsx('border-l-4 rounded-r p-3 text-sm space-y-1', color)}>
      <div className="flex items-center gap-2 font-semibold">
        ...
        <span className="text-xs text-gray-500">[{obj.objection_id}]</span>
        {obj.resolved && (
          <span className="ml-auto text-xs text-green-600 font-normal">Resolved ✓</span>
        )}
      </div>
      <p className="text-gray-800">{obj.description}</p>
      <p className="text-gray-600 italic text-xs">→ {obj.recommendation}</p>
      {obj.citation && (
        <p className="text-gray-400 text-xs">Source: {obj.citation}</p>
      )}
    </div>
  )
}
